        ...
        if noisyDistance is None:
            ...
            for legalPosition in self.legalPositions:
                ...
                if emissionModel[trueDistance] > 0:
                    allPossible[legalPosition] = emissionModel[trueDistance] * self.beliefs[legalPosition]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for legalPosition in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                updatedBeliefs[newPos] += prob * self.beliefs[legalPosition]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while numParticles > 0:
            ...
            for legalPos in self.legalPositions:
                uniformParticles.append(legalPos)
        self.particles = uniformParticles
    ...
    def observe(self, observation, gameState):
        ...
        uniformParticles = []
        allParticlesWeight = util.Counter()
        if noisyDistance is None:
            # numParticles = self.numParticles
            # while numParticles > 0:
            #     numParticles -= 1
            ...
            for particle in self.particles:
                ...
                allParticlesWeight[particle] += emissionModel[trueDistance]
                # *  self.getBeliefDistribution()[legalPosition]

            allParticlesWeight.normalize()
            if allParticlesWeight.totalCount() == 0:
                ...
                while numParticles > 0:
                    numParticles -= 1
                    uniformParticles.append(util.sample(allParticlesWeight))
                self.particles = uniformParticles
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        weight with each position) is incorrect and may produce errors.
        """
        ghostPermutations = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(ghostPermutations)
        numParticles = self.numParticles
        particleList = []
        while numParticles > 0 and ghostPermutations:
            numParticles -= 1
            particleList.append(ghostPermutations.pop())
    ...
    def observeState(self, gameState):
        ...
        allParticleWeights = util.Counter()
        updatedParticleList = []
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i] is None:
                tempList = []
                for particle in self.particles:
                    tempList.append(self.getParticleWithGhostInJail(particle, i))
        ...
        for particle in self.particles:
            ...
            allParticleWeights[particle] += particleWeight

        allParticleWeights.normalize()
        if allParticleWeights.totalCount() == 0:
            self.initializeParticles()
        else:
            numParticles = self.numParticles
            while numParticles > 0:
                numParticles -= 1
                updatedParticleList.append(util.sample(allParticleWeights))
            self.particles = updatedParticleList
